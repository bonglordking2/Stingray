...
local Highlight = {"5 Demon Finger", "Maximum Scroll", "Domain Shard", "Iridescent Lotus", "Polished Beckoning Cat",
                   ...
                   "Rotten Chains"}
local Curses = {"Low Level Curse","Grade 4 Curse", "Grade 3 Curse", "Grade 2 Curse", "Grade 1 Curse", "Special Grade Curse"}
...
if Toggle == "ON" then
    local Queued, QueueFail = pcall(function()
        queue_on_teleport('loadstring(game:HttpGet("https://raw.githubusercontent.com/Nebula-Manta/Stingray/refs/heads/main/JJI/InvestigationFarm.lua"))()')()
    end)
    if not Queued then
...
local function TweenTo(CF)
    ...
    local Tween = game:GetService("TweenService"):Create(Root, TweenInfo.new(Distance / 200, Enum.EasingStyle.Linear), {
        CFrame = CF
    })
    Tween:Play()
...
local function Target(Humanoid)
    ...
    if not s then print("Aim Hook Failure: "..e) end
end
    ...
    while not Replay.Visible do
        ...
        pcall(function()
            ...
            elseif Mobs:FindFirstChild("QuestMarker", true) then
                Godmode(false)
                local Quest = Mobs:FindFirstChild("QuestMarker", true)
                if table.find(Curses, Quest.Adornee.Parent.Name) then
                    print(Quest.Adornee)
                    Kill(Quest.Adornee.Parent)
                end
            else
